...
type RestOf<T extends IActivity | NewActivity> = Omit<T, 'publishedDate' | 'bookedDate'> & {
...
export class ActivityService {
  ...
  protected convertDateFromClient<T extends IActivity | NewActivity | PartialUpdateActivity>(activity: T): RestOf<T> {
    return {
      ...activity,
      publishedDate: activity.publishedDate?.toJSON() ?? null,
      bookedDate: activity.bookedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restActivity: RestActivity): IActivity {
    return {
      ...restActivity,
      publishedDate: restActivity.publishedDate ? dayjs(restActivity.publishedDate) : undefined,
      bookedDate: restActivity.bookedDate ? dayjs(restActivity.bookedDate) : undefined,
    };
  }
